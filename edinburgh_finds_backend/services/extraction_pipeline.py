...
def merge_source_info(llm_info: dict, source_type: str) -> dict:
    """
    Merge LLM-provided source_info with system provenance.
    - Deduplicate URLs
    - Never trust non-URL strings
    - Always append system provenance
    """
    system_provenance = {
        "sources": [source_type],
        "note": f"Processed by extraction on {datetime.now():%Y-%m-%d %H:%M:%S}"
    }

    if not llm_info:
        return system_provenance

    # Extract valid URLs only (safety guard)
    llm_urls = llm_info.get("sources", [])
    llm_urls = [
        url for url in llm_urls
    ...
    system_note = system_provenance["note"]
    merged_note = system_note if not llm_note else f"{llm_note}; {system_note}"
...
def process_raw_text(
    ...
    # -----------------------------------------------------
    dto.source_info = merge_source_info(dto.source_info, source_type)
